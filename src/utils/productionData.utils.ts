export class ProductionDataUtils {
  ...
  public unravel(barcode: string) {
    ...
    const indexOfMachine = availableMachines.findIndex((element: string) =>
      barcode.includes(element),
    );
    ...
    const lastIndexMachine =
      indexMachine + availableMachines[indexOfMachine]!.length;
